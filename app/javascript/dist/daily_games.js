...
class Horde extends Position {
    ...
    hasInsufficientMaterial(color) {
        ...
        const pieces = MaterialSide.fromBoard(this.board, opposite$1(color));
        const piecesBishops = (squareColor) => coloredSquares(squareColor)
            .intersect(this.board.pieces(opposite$1(color), 'bishop'))
            .size();
            ...
            if (piecesNum === 1) {
                ...
                (piecesNum >= 4 &&
                    (pieces.knight >= 2 ||
                        ...
                        (hasBishopPair(opposite$1(color)) && pieces.pawn >= 1)) &&
    ...
    for (const key in extend) {
        if (Object.prototype.hasOwnProperty.call(extend, key)) {
            if (Object.prototype.hasOwnProperty.call(base, key) &&
                isPlainObject$1(base[key]) &&
                isPlainObject$1(extend[key]))
...
function createDefs() {
    const defs = createElement$1('defs');
    const filter = setAttributes(createElement$1('filter'), { id: 'cg-filter-blur' });
    filter.appendChild(setAttributes(createElement$1('feGaussianBlur'), { stdDeviation: '0.019' }));
...
function renderShape$1(state, { shape, current, hash }, brushes, dests, bounds) {
    ...
    if (brush) {
        const el = setAttributes(createElement$1('g'), { cgHash: hash });
        svgs.push({ el });
    ...
    if (shape.customSvg) {
        ...
        const el = setAttributes(createElement$1('g'), { transform: `translate(${x},${y})`, cgHash: hash });
...
function renderArrow(s, brush, from, to, current, shorten) {
    ...
    const g = createElement$1('g');
    const blurred = setAttributes(createElement$1('g'), { filter: 'url(#cg-filter-blur)' });
...
function renderLabel(label, hash, from, to, slots, corner) {
    var _a;
    const labelSize = 0.4, fontSize = labelSize * 0.75 ** label.text.length, at = labelCoords(from, to, slots), cornerOff = corner === 'tr' ? 0.4 : 0, g = setAttributes(createElement$1('g'), {
        transform: `translate(${at[0] + cornerOff},${at[1] - cornerOff})`,
        cgHash: hash,
    });
    g.appendChild(setAttributes(createElement$1('circle'), {
        r: labelSize / 2,
    ...
    const labelEl = setAttributes(createElement$1('text'), {
        'font-size': fontSize,
...
function filterBox(from, to) {
    ...
    return setAttributes(createElement$1('rect'), {
        x: box.from[0],
        y: box.from[1],
...
function create_fragment$1(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(div0, file$1, 51, 6, 1348);
			attr_dev(div1, "class", "cell");
			add_location(div1, file$1, 50, 4, 1323);
			attr_dev(div2, "class", "is2d reversed");
			attr_dev(div2, "id", div2_id_value = "" + (/*game*/ ctx[0].url + "-reversed"));
			add_location(div2, file$1, 54, 6, 1458);
			attr_dev(div3, "class", "cell");
			add_location(div3, file$1, 53, 4, 1433);
			attr_dev(div4, "class", "grid");
			add_location(div4, file$1, 49, 2, 1300);
			add_location(hr, file$1, 57, 2, 1565);
			attr_dev(div5, "class", "fixed-grid has-2-cols");
			add_location(div5, file$1, 48, 0, 1262);
		},
		...
		p: function update(ctx, [dirty]) {
			...
			if (dirty & /*game*/ 1 && div2_id_value !== (div2_id_value = "" + (/*game*/ ctx[0].url + "-reversed"))) {
...
function instance$1($$self, $$props, $$invalidate) {
	...
	onMount(() => {
		pgnViewer = start(pgnViewerContainer, {
			pgn: game.pgn,
			initialPly: 'last',
			orientation: myColor,
			scrollToMove: false
		...
		reversedViewer = start(reversedBoard, {
			pgn: game.pgn,
			initialPly: 'last',
	...
	return [game, pgnViewerContainer, reversedBoard, myColor, div0_binding, div2_binding];
}

class DailyGame extends SvelteComponentDev {
	constructor(options) {
		super(options);
		init$1(this, options, instance$1, create_fragment$1, safe_not_equal, { game: 0, myColor: 3 });
